...
def main():
    ...
    # SEPARATE TRAINING DATASET FOR 1st STAGE AND 2nd STAGE (Blending)
    ...
    if approach == 0:
        ...
        model, pca = reg.tune_linear_model(x_train, y_train, k_fold)
        nameModel = 'linear_reg'
        if pca !=-1:
            x_test = pca.transform(x_test)

    elif approach == 5:
        model = reg.tune_grad_boost_regressor(x_train, y_train, k_fold)
...
#    print(model.feature_importances_)
    ...
    # Save model
    joblib.dump((model, pca), 'models/'+nameModel+'.pkl')
